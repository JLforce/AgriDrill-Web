...
export default function MagazinePage() {
  ...
  const [activeTray, setActiveTray] = useState(0); // index in trays
  const [stepperPos, setStepperPos] = useState({ row: 0, col: 0 });
  const rows = 4, columns = 6; // Example tray grid size

  // Calculate total seedlings
  const totalSeedlings = trays.reduce((sum, t) => sum + t.seedlings, 0);

  // Handlers
  const handleAdvanceTray = () => {
    // Drop current tray, activate next (wrap around)
    setActiveTray((prev) => (prev + 1) % trays.length);
    setStepperPos({ row: 0, col: 0 });
  };
  const handleIndexReset = () => setStepperPos({ row: 0, col: 0 });
  const handleReloadTray = () => {
